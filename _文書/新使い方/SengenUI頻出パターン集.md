    ...
    protected createComponentRoot(): DivC {
        return (
            div({ class: styles.root }).childs([
                div({ class: styles.header }).child(
                    button({ text: "追加" }).onClick(() => this._配線.先.on追加())),
                this._リストコンテナ])
        );
    }
...
| 派生値 | FP（派生セル） | 宣言的 |
| View構築 | 宣言的メソッドチェーン（LV1拡張 / LV2素部品） | ツリーが1つの式で読める |
...
class チャットOrchestrator extends LV2HtmlComponentBase implements I破棄可能 {
    ...
    protected createComponentRoot(): DivC {
        return (
            div({ class: styles.root }).childs([
                this._メッセージリスト,
                // 入力フォームは第4章の検索フォームViewと同型の素部品。配置しながら配線する
                new チャット入力フォーム().配線する({
