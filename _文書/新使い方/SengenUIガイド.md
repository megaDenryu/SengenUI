...
## 第3条：View / Orchestrator 二層アーキテクチャ

コンポーネントは **View層**（純粋関数）と **Orchestrator層**（クラス）に分離する。

| 層 | 形式 | 責務 | 状態 |
|---|---|---|---|
| **View層** | 純粋関数 | UIツリーの構築と返却 | 持たない |
| **Orchestrator層** | LV2クラス | 状態管理・イベントハンドリング | 持つ |
...
- `LV2HtmlComponentBase` を継承する。
- `createComponentRoot()` 内でView関数を呼び出してUIツリーを構築する。
- `buildUI()` のような第二初期化メソッドは禁止。`createComponentRoot()` で完結。
