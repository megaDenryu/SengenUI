...
Why（素部品と部品集約を分ける理由）: 子にLV2部品を含むと「子の配線を集約する」責務が発生するため、部品DTO+`_ルートを構築する(部品)` の型契約（第4条）を強制する。子がLV1だけなら配線の集約は発生せず、部品DTOの強制はボイラープレートが利得を上回るため従来のLV2で作る。素部品も部品集約も `I配線可能` を実装すれば「使われる部品」になれる（部品集約は部品でもありうる）。
