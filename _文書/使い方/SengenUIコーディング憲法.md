...
- TypeScriptの特にSengenUIを用いたUIコンポーネントの書き方については以下に従ってください。**Dom操作は禁止。Dom操作は禁止。Dom操作は禁止。Dom操作は禁止。** 
    ...
    - **LV2コンポーネント継承の禁止**: `LV2HtmlComponentBase` を直接継承した具象コンポーネントクラスを、さらに継承して新しいクラスを作ることは禁止です（Sealed原則）。`createComponentRoot()` の実行タイミングが複雑になり、初期化前のフィールドアクセス等のバグを防ぐためです。拡張にはコンポジション（包含）を使用してください。
