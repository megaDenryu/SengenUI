 ...
 * 型で明示すべきである。_ルートを構築する(部品, サービス) を abstract で強要することで、
 * ルート構築が this 経由の暗黙依存ではなく、引数リストに列挙された部品だけから
 * ツリーを作ることが型契約になる（スコープで見えるかではなく、依存の明示が目的）。
 * サービスが不要な集約は Tサービス を省略し、実装側の引数を部品だけにしてよい。
 *
 * 呼び出しはサブクラス自身のコンストラクタの最終段階で行う:
 *   this._componentRoot = this._ルートを構築する(部品, サービス);
 ...
 */
export abstract class LV2部品集約Base<T部品, Tサービス = void> extends LV2HtmlComponentBase {
    protected abstract _ルートを構築する(部品: T部品, サービス: Tサービス): HtmlComponentBase;
}
